...
function renderTimerCard(timer) {
    ...
    div.innerHTML = `
        ...
        <div class="timer-original">
            ...
            ${formatDuration(timer.durationDays, timer.durationHours, timer.durationMinutes)}
        </div>
        <div class="countdown ${countdownClass}" data-id="${timer.id}">
            ${formatCountdown(remaining)}
        </div>
        <div class="timer-controls">
            ${canStart ? `<button onclick="startTimer('${timer.id}')"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5,3 19,12 5,21"></polygon></svg> START</button>` : ''}
            ${canPause ? `<button class="btn-pause" onclick="pauseTimer('${timer.id}')"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg> PAUSE</button>` : ''}
            ${canResume ? `<button class="btn-resume" onclick="resumeTimer('${timer.id}')"><svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><polygon points="5,3 19,12 5,21"></polygon></svg> RESUME</button>` : ''}
            <button class="btn-secondary btn-edit" onclick="openEditModal('${timer.id}')" title="Edit">
